...
/**
 * Boshqaruv panelidagi "Vazifalar" bo'limi.
 *
 ...
 */
export default function TasksPanel({ title }) {
  ...
  return (
    <div className="card">
      <div className="panel-head">
        <h2>📝 {title || t('tasks.title')}</h2>
        <span className="panel-head__meta">{t('dash.total', { n: data.total })}</span>
        {counts.review > 0 && (
          <span className="panel-head__meta" style={{ color: 'var(--warning)' }}>
            {t('dash.count.review')}: <strong>{counts.review}</strong>
          </span>
        )}
        {counts.returned > 0 && (
          <span className="panel-head__meta">
            {t('dash.count.returned')}: <strong>{counts.returned}</strong>
          </span>
        )}
        {counts.overdue > 0 && (
          <span className="panel-head__meta" style={{ color: 'var(--danger)' }}>
            {t('dash.count.overdue')}: <strong>{counts.overdue}</strong>
          </span>
        )}
      </div>
