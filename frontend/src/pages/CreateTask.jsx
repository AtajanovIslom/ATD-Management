  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (form.assign_type === 'individual' && assigneeIds.length === 0) {
        setError(t('task.err.pickWorker'))
        setLoading(false)
        return
      }
        ...
        <form onSubmit={handleSubmit}>
          <div className="form-group">
            ...
            <input className="form-input" value={form.name}
              ...
              placeholder={t('task.field.name.placeholder')} required />
          </div>
          ...
          <div className="form-group">
            ...
            <textarea className="form-input" value={form.description}
              ...
              placeholder={t('task.field.description.placeholder')} rows={3} />
          </div>
            ...
            {form.assign_type === 'team' && (
              <>
                <select className="form-input" value={form.team_id}
                  ...
                  <option value="">{t('task.selectTeam')}</option>
                  {teams.map(team => (
                ...
                {form.team_id && members.length > 0 && (
                  <div style={{ marginTop: 8 }}>
                    ...
                    <select className="form-input" value={form.assignee_id}
                      ...
                      <option value="">{t('task.everyoneResponsible')}</option>
                      {members.map(m => (
              ...
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                  <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                    {t('task.selectedCount', { n: form.assignee_ids.length, total: users.length })}
                  </span>
                  <button type="button" className="btn btn-outline btn-sm" onClick={toggleAll}>
                    {form.assignee_ids.length === users.length && users.length > 0 ? t('task.deselectAll') : t('task.selectAll')}
                  </button>
                </div>
          ...
          <button type="submit" className="btn btn-primary btn-full" disabled={loading}>
            {loading ? t('btn.creating') : t('task.create.submit')}
          </button>
        </form>
      </div>
    </div>
